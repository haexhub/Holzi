...
def upgrade() -> None:
    ...
    # Hand-added: per-user partial unique index on llm_credentials. Autogenerate
    # cannot infer this from `metadata` (it's a Postgres-specific partial index
    # rather than a SQLAlchemy UniqueConstraint).
    op.create_index(
...
def downgrade() -> None:
    """Downgrade schema."""
    # Hand-added: drop the per-user partial unique index before dropping its
    # table.
