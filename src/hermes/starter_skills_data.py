"""Seed data for starter skills.

Extracted from ``hermes.starter_skills`` to keep the loader file under
the project's 500-LoC ceiling. Pure data — the loader in
``starter_skills.py`` imports ``STARTER_SKILLS`` and inserts each row
via ``INSERT ... ON CONFLICT DO NOTHING`` on boot.

Adding a new skill: append a dict here, that's it.
"""
