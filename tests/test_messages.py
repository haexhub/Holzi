...
async def test_fts_search_finds_matching_messages(
    ...
    hits = await messages.fts_search(conn, user_id=1, query="meeting")
...
async def test_fts_search_can_be_scoped_to_conversation(
    ...
    convo_b = await conversations.create(conn, user_id=1, channel="web", ts=2)
    await messages.append(
        conn, user_id=1, conversation_id=convo_a.id, role="user", content="meeting at noon", ts=10
    )
    await messages.append(
        conn, user_id=1, conversation_id=convo_b.id, role="user", content="meeting tomorrow", ts=20
    )

    hits = await messages.fts_search(conn, user_id=1, query="meeting", conversation_id=convo_a.id)
...
async def test_fts_index_updates_when_message_is_deleted(
    ...
    msg = await messages.append(
        conn, user_id=1, conversation_id=convo_id, role="user", content="zorblax", ts=10
    )
    from sqlalchemy import text as _text

    # Raw DELETE must run under the row's RLS context, else the FORCEd
    # policy (GUC default '0') silently blocks it and the row survives.
    async with conn.begin() as raw:
        await raw.execute(_text("SELECT set_config('app.user_id', '1', true)"))
        await raw.execute(
            _text("DELETE FROM messages WHERE id = :id"), {"id": msg.id}
        )

    hits = await messages.fts_search(conn, user_id=1, query="zorblax")
    assert hits == []
...
async def test_last_user_message_returns_most_recent_user_turn(
    ...
    last = await messages.append(
        conn, user_id=1, conversation_id=convo_id, role="user", content="second", ts=30
    )
    await messages.append(
        conn, user_id=1, conversation_id=convo_id, role="assistant", content="reply 2", ts=40
    )

    found = await messages.last_user_message(conn, convo_id, user_id=1)
...
async def test_last_user_message_returns_none_when_no_user_turn(
    ...
    await messages.append(
        conn, user_id=1, conversation_id=convo_id, role="assistant", content="orphan", ts=10
    )
    assert await messages.last_user_message(conn, convo_id, user_id=1) is None
...
async def test_delete_after_removes_trailing_messages(
    ...
    deleted = await messages.delete_after(conn, convo_id, user_id=1, after_id=user.id)
    assert deleted == 3

    remaining = await messages.list_by_conversation(conn, convo_id, user_id=1)
...
async def test_delete_after_keeps_fts_index_in_sync(
    ...
    user = await messages.append(
        conn, user_id=1, conversation_id=convo_id, role="user", content="keep me", ts=10
    )
    await messages.append(
        conn, user_id=1, conversation_id=convo_id, role="assistant", content="zorblax", ts=20
    )

    await messages.delete_after(conn, convo_id, user_id=1, after_id=user.id)

    assert await messages.fts_search(conn, user_id=1, query="zorblax") == []
    assert len(await messages.fts_search(conn, user_id=1, query="keep")) == 1
...
async def test_delete_after_is_scoped_to_conversation(conn: AsyncEngine) -> None:
    ...
    user_a = await messages.append(
        conn, user_id=1, conversation_id=convo_a.id, role="user", content="a-user", ts=10
    )
    await messages.append(
        conn, user_id=1, conversation_id=convo_a.id, role="assistant", content="a-reply", ts=20
    )
    await messages.append(
        conn, user_id=1, conversation_id=convo_b.id, role="assistant", content="b-reply", ts=30
    )

    deleted = await messages.delete_after(conn, convo_a.id, user_id=1, after_id=user_a.id)
    assert deleted == 1
    b_msgs = await messages.list_by_conversation(conn, convo_b.id, user_id=1)
    assert [m.content for m in b_msgs] == ["b-reply"]


async def test_get_returns_message_by_id(conn: AsyncEngine, convo_id: int) -> None:
    msg = await messages.append(
        conn, user_id=1, conversation_id=convo_id, role="user", content="find me", ts=10
    )
    found = await messages.get(conn, msg.id, user_id=1)
    assert found is not None
...
async def test_get_returns_none_for_unknown_id(conn: AsyncEngine) -> None:
    assert await messages.get(conn, 99999, user_id=1) is None
...
async def test_update_content_replaces_text_and_keeps_position(
    ...
    msg = await messages.append(
        conn, user_id=1, conversation_id=convo_id, role="user", content="old text", ts=10
    )
    updated = await messages.update_content(conn, msg.id, user_id=1, content="new text")
...
async def test_update_content_keeps_fts_index_in_sync(
    ...
    await messages.update_content(conn, msg.id, user_id=1, content="quuxified")
    assert await messages.fts_search(conn, user_id=1, query="zorblax") == []
    assert len(await messages.fts_search(conn, user_id=1, query="quuxified")) == 1


async def test_update_content_returns_none_for_unknown_id(conn: AsyncEngine) -> None:
    assert await messages.update_content(conn, 99999, user_id=1, content="x") is None
