...
async def test_upsert_inserts_new_note(conn: AsyncConnection) -> None:
...
async def test_upsert_updates_existing_note_keeping_id(conn: AsyncConnection) -> None:
...
async def test_get_returns_none_for_missing_key(conn: AsyncConnection) -> None:
    assert await notes.get(conn, "does-not-exist") is None


async def test_find_searches_via_fts_on_content(conn: AsyncConnection) -> None:
...
async def test_find_matches_against_tags(conn: AsyncConnection) -> None:
...
async def test_fts_index_updates_on_upsert_overwrite(conn: AsyncConnection) -> None:
