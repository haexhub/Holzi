...
async def test_recall_memory_respects_limit(conn: aiosqlite.Connection) -> None:
    convo = await conversations.create(conn, channel="signal", ts=1)
    for i in range(5):
        await messages.append(
...
async def test_find_notes_returns_matches(conn: aiosqlite.Connection) -> None:
...
async def test_find_notes_filters_by_tags(conn: aiosqlite.Connection) -> None:
