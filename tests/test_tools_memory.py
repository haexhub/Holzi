...
async def test_recall_memory_finds_messages_and_notes(
    ...
    await messages.append(
        conn, user_id=1, conversation_id=convo.id, role="user", content="reschedule standup", ts=10
    )
    await messages.append(
        conn, user_id=1, conversation_id=convo.id, role="user", content="buy milk", ts=20
    )
    await notes.upsert(
...
async def test_recall_memory_respects_limit(conn: AsyncEngine) -> None:
    ...
    for i in range(5):
        await messages.append(
            conn, user_id=1, conversation_id=convo.id, role="user", content="alpha", ts=i + 10
        )
...
async def test_list_conversations_returns_all_with_message_count(
    ...
    assert by_id[a.id]["channel"] == "task"
...
async def test_list_conversations_can_filter_by_channel(
    ...
    data = json.loads(payload)
    assert {c["channel"] for c in data} == {"task"}
    assert len(data) == 2
...
async def test_get_conversation_returns_metadata_and_messages(
    ...
    await messages.append(conn, user_id=1, conversation_id=convo.id, role="user", content="a", ts=10)
    await messages.append(conn, user_id=1, conversation_id=convo.id, role="assistant", content="b", ts=11)
